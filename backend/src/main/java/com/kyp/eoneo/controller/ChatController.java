...
//    /publish/chat/message
    ...
    public ResponseEntity<CommonResult> updateUnreadMessage(
            @ApiParam(
                    name =  "disconnect할 때, api 요청을 해야해요. 그래서 읽은 메세지를 다 배열에 저장해서 주세요!!",
                    example =   " 'messages' : [25, 26]",
                    required = true)
            @RequestBody Map<String, List<Long>> messages, @PathVariable String roomId){
